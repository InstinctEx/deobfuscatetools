    ...
    public byte[] MakeCode(byte[] data)
    {
    	if (ReadLicenseKey.keyAd==null||ReadLicenseKey.keyAd.Length==0)
            ...
            using (MemoryStream stream1 = new MemoryStream())
            {
                ...
                {
                	byte[] asa = ReadLicenseKey.keyAd;
                    rijndael1.Key = ReadLicenseKey.keyAd;
                    rijndael1.IV = new byte[0x10];
                    using (CryptoStream stream2 = new CryptoStream(stream1, rijndael1.CreateEncryptor(), CryptoStreamMode.Write))
                    {
